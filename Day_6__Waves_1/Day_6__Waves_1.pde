...
public class Lissajous {
  ...
  public void draw() {
    ...
    float py = cos(angle*3) * length - angle;
    point(px, py);
...
Lissajous[] lissajousArr = {
  ...
  new Lissajous(width/2, height/2, magenta, (angle + 0.12 * pointCount) % (PI * 2)), 
  new Lissajous(width/2, height/2, yellow, (angle + 0.12 * pointCount) % (PI * 2)), 
};
